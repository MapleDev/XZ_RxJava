...
public class HttpMethods {
     ...
     * @param count 获取长度
     */
    public void getTopMovie(Subscriber<List<Subjects>> subscriber, int start, int count){
        movieService.getTopMovie(start, count)
//                .map(new HttpResultFunc<List<Subjects>>())
                .map(new Func1<HttpResult<List<Subjects>>, List<Subjects>>() {
                    @Override
                    public List<Subjects> call(HttpResult<List<Subjects>> listHttpResult) {
